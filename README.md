...
1. покажет профили и предложит тот, где стоят мосты; **мосты возьмёт из панели** — добавили ноду, сторож её подхватит;
2. найдёт клиентский инбаунд, прочитает `sni` и `shortId`, посчитает публичный ключ Reality;
3. **заведёт служебного пользователя** `bridge-probe` и положит его в сквад этого инбаунда;
...
| ключ | что это |
|---|---|
| `bridges` | `"auto"` — ноды профиля `panel.profile` из панели каждый прогон; или список `[{name, ip}]` |
